        ...
        for line in lines:
            ...
            if len(formatted_data) == 0:
                ...
                for box in formatted_data:
                    ...
                    assert len(attributes) == 2 and 'occluded' in attributes and \
...
output_json_fn = os.path.expanduser('~/data/noaa-fish-lila/noaa_estuary_fish.json')
base_folder = os.path.expanduser('~/data/noaa-fish-lila')
jpeg_image_folder = 'JPEGImages'
...
html_output_file, _ = visualize_db.process_images(db_path=output_json_fn,
                                                         ...
                                                         image_base_dir=os.path.join(base_folder,
                                                         jpeg_image_folder),
                                                         options=viz_options)
...
original_json_fn = os.path.expanduser('~/data/noaa-fish-lila/noaa_estuary_fish.json')
filter_xlsx = '/home/user/Downloads/filteredimages.xlsx'
filter_json_fn = os.path.expanduser('~/data/noaa-fish-lila/noaa_estuary_fish_filter.json')

with open(original_json_fn,'r') as f:
    d = json.load(f)
    
fn_to_image = {}
for im in d['images']:
    fn_to_image[im['file_name']] = im
    
import pandas as pd
df = pd.read_excel(filter_xlsx)
filtered_images = df['My List'].values.tolist()

# fn = filtered_images[0]
for fn in filtered_images:
    assert fn in fn_to_image
filtered_images_set = set(filtered_images)

n_filtered = 0

for im in d['images']:
    if im['file_name'] in filtered_images_set:
        im['filter'] = True
        n_filtered +=1
    else:
        im['filter'] = False
        
print('Applied the "filter" property to {} of {} images'.format(n_filtered,len(d['images'])))

d['info']['version'] = '2023.03.10.00'
