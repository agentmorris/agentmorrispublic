#
# Code to render sample images and annotations in the Mou et al. drone dataset (WAID):
#
# https://github.com/xiaohuicui/WAID/tree/main/WAID
#
...
base_folder = r'c:\git\WAID\WAID'
output_file_annotated = r'c:\temp\waid_sample_image_annotated.jpg'
output_file_unannotated = r'c:\temp\waid_sample_image_unannotated.jpg'
...
# In practice:
#
# {0: 'sheep', 1: 'cattle', 2: 'seal', 3: 'camelus', 4: 'kiang', 5: 'zebra'}    


#%% Read and summarize annotations

annotation_files_relative = \
    path_utils.recursive_file_list(labels_folder,return_relative_paths=True,convert_slashes=True)
...
for annotation_file_relative in tqdm(annotation_files_relative):
    ...
    image_filename_abs = os.path.join(images_folder,image_filename_relative)
    annotation_filename_abs = os.path.join(labels_folder,annotation_file_relative)
...
image_relative_path = sorted_annotations[4000]

image_full_path = os.path.join(images_folder,image_relative_path)
...
visutils.draw_bounding_boxes_on_file(image_full_path, output_file_annotated, detection_formatted_boxes,       
                                     confidence_threshold=0.0,detector_label_map=category_id_str_to_name,
